...
class LSTM_vowel_recognizer_larger(nn.Module):
    def __init__(self, hidden_dim=256, win_length=12, num_lstm_layer=3):
        super(LSTM_vowel_recognizer_larger, self).__init__()
        # here I'm going to assume the input is gonna be shaped as
        self.hidden_dim = hidden_dim
        self.win_length = win_length
        self.num_lstm_layer = num_lstm_layer
...
if __name__ == "__main__":
    ...
    # for re-training
    model.load_state_dict(torch.load("../../Dataset/viseme_net_model_larger_model/model_epoch_650.pt")['model_state_dict'])
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.00001)
    # for re-training
    optimizer.load_state_dict(torch.load("../../Dataset/viseme_net_model_larger_model/model_epoch_650.pt")['optimizer_state_dict'])
    epochs = 10000
