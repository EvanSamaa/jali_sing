...
def compute_error(points_vocal, points_mocap):
    ...
    return base_error 
def compute_window_error(points_vocal, points_mocap, t, window_length):
    base_error = 0
    for tt in [t, t + math.ceil(window_length/2)]:
        cmds.currentTime(tt)
        for i in range(0, len(points_vocal)):
            pos1 = cmds.pointPosition(points_vocal[i])
            pos2 = cmds.pointPosition(points_mocap[i])
...
def optimize_one_time_step(points_vocal, points_mocap, t):
    ...
    for i in range(0, 3):
        ...
        if error_1 <= error_2:
            ...
            minmin = slider_max
    cmds.setKeyframe(slider+".ty", v=minmin, t=t)
    ...
    print("for MAPS: time {} is done".format(t))
def optimize_one_time_step_JALI(points_vocal, points_mocap, t, win_len):
    curve_suffix = ["_M_P", "_Mnot_P", "_M_Pnot", "_Mnot_Pnot"]
    slider = "JaliJoystick_Jaw"
    slider_min = 0
    slider_max = 10
    for i in range(0, 3):
        for suffix in curve_suffix:            
            cmds.setKeyframe(slider+suffix , v=slider_min, t=t)
        error_1 = compute_window_error(points_vocal, points_mocap, t, win_len)
        for suffix in curve_suffix:            
            cmds.setKeyframe(slider+suffix , v=slider_max, t=t)
        error_2 = compute_window_error(points_vocal, points_mocap, t, win_len)
        if error_1 <= error_2:
            slider_max = (slider_max + slider_min)/2.0
        else:
            slider_min = (slider_max + slider_min)/2.0
    for suffix in curve_suffix:
        cmds.setKeyframe("JaliJoystick_Jaw" + suffix, v=(slider_max + slider_min)/2.0, t=t)
        
    slider = "JaliJoystick_Lip"
    slider_min = 0
    slider_max = 10
    for i in range(0, 3):
        for suffix in curve_suffix:            
        ...
        if error_1 <= error_2:
            ...
            slider_min = (slider_max + slider_min)/2.0
    for suffix in curve_suffix:
        cmds.setKeyframe("JaliJoystick_Lip" + suffix, v=(slider_max + slider_min)/2.0, t=t) 
    print("for JALI: time {} is done".format(t))
    
    
def optimize_vocal_curve(start_frame, end_frame, delta_VOCAL = 1, delta_JALI=5, bool_optimize_JALI=True):
    ...
    # optimize JALI curve
    if bool_optimize_JALI:
        JA_CURVES = []
        # first store the previous values of the JALI curves, with consideration to vibrato
        curve_suffix = ["_M_P", "_Mnot_P", "_M_Pnot", "_Mnot_Pnot"]
        for suffix in curve_suffix:
            JA_CURVES.append([])
            JA_keys = cmds.keyframe("JaliJoystick_Jaw"+suffix, query=True)
            base_val = cmds.keyframe("JaliJoystick_Jaw"+suffix, query=True, eval=True, time=(0,0))[0]
            for key_t in JA_keys:
                val = cmds.keyframe("JaliJoystick_Jaw"+suffix, query=True, eval=True, time=(key_t,key_t))[0]
                JA_CURVES[-1].append([key_t, val-base_val])
        # delete all the keys
        for suffix in curve_suffix:
            cmds.cutKey("JaliJoystick_Jaw"+suffix)
        init_JALI_JOY_STICK()
        for t in range(start_frame, end_frame+1):
            if t%delta_JALI == 0:
                optimize_one_time_step_JALI(points_vocal, points_mocap, t, delta_JALI)
        # optimize VOCAL curves
        
        for ja in range(0, 4):
            JA_CURVE = JA_CURVES[ja]
            slider = "JaliJoystick_Jaw"+curve_suffix[ja]
            original_vals = []
            for ttt in range(0, len(JA_CURVE)):
                original_val = cmds.keyframe(slider, query=True, eval=True, time=(JA_CURVE[ttt][0],JA_CURVE[ttt][0]))[0]
                original_vals.append(original_val)
            for ttt in range(1, len(JA_CURVE)):
                cmds.setKeyframe(slider, v=JA_CURVE[ttt][1]+original_vals[ttt], t=JA_CURVE[ttt][0])
    for t in range(start_frame, end_frame+1):
        if t%delta_VOCAL == 0:
            cmds.currentTime(t)
...
optimize_vocal_curve(398, 1053, delta_VOCAL=15, delta_JALI=15, bool_optimize_JALI=True)
