...
def differenceFunction(x, N, tau_max):
    ...
    return np.expand_dims(x_cumsum[w:w - tau_max:-1], axis=1) + x_cumsum[w] - np.expand_dims(x_cumsum[:tau_max], axis=1) - 2 * conv
def cumulativeMeanNormalizedDifferenceFunction(df, N):
    ...
    # print()
    # A[2]
    # potentially need to change cumsum axis
    cmndf = df[1:] * np.expand_dims(np.array(range(1, N)), axis=1) / np.cumsum(df[1:], axis=0).astype(float) #scipy method
...
def yinAlgo(audioFileName="whereIam.wav", w_len=1024, w_step=256, f0_min=70, f0_max=200, harmo_thresh=0.85, audioDir=None, dataFileName=None, verbose=4):
    """
...
if __name__ == '__main__':
    pitches, harmonic_rates, argmins, times = yinAlgo()
